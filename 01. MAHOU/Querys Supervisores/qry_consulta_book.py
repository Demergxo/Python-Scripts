...
query = text("""
    SELECT
        ID_Doc, CodigoTipoDocumento AS [Tipo Documento],AlbaranDoc AS [Pedido MSM],
        PaletsClienteDoc AS [Palets Estimados], CampoCliente AS [Codigo Doc], PedidoDestinatarioDoc AS [Pedido Cliente]
    FROM
        vDocumentos
    WHERE 
        ID_Cliente = 944
...
df["Tipo Documento"] = df["Tipo Documento"].map({
    "ALB": "Cargas",
    "RS": "Descargas"
}).fillna(df["Tipo Documento"])
...
# Dividimos por fecha y hora
df["Fecha"] = pd.to_datetime(df["Fecha Carga"]).dt.strftime("%Y-%m-%d")
df["Hora"] = pd.to_datetime(df["Fecha Carga"]).dt.strftime("%H:%M:%S")
...
df_final = df.groupby("Pedido MSM", as_index=False).agg({
    ...
    "Pedido Cliente": "first",
    "Palets Estimados": "first"

...
def consulta_XPO(fecha_inicio, hora_inicio, fecha_fin, hora_fin):
    ...
    query = text("""
        ...
        '67248703-GXO LOGISTICS SPAIN SLU' AS 'Sub-Account', 'SAN FERNANDO' AS 'Depot Information', RTRIM(AlbaranDoc) AS 'Customer Reference',
    ...
    WHERE 
        ...
        AND ID_Almacen = 220
        AND CONVERT(datetime, FechaProcesoDoc) BETWEEN CONVERT(date, :inicio) AND CONVERT(date, :fin)
...
# --- EJECUTAR CONSULTA SQL SERVER ---
    fecha_inicio_c = fecha_inicio + " " + hora_inicio_str
    fecha_fin_c = fecha_fin + " " + hora_fin_str

    with engine.connect() as conn:
        df_doc = pd.read_sql(query, conn, params={"inicio": fecha_inicio_c, "fin": fecha_fin_c})
        df_pend = pd.read_sql(query_pend, conn, params={"inicio": fecha_inicio_c, "fin": fecha_fin_c}) #type:ignore // falla si no pasa desde main por el opjeto Pyqt6.time 
...
if __name__ == "__main__":
    consulta_XPO("2026-01-19", "16:00:00", "2026-01-20", "16:00:00")
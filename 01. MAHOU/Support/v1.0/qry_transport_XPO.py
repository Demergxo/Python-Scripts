...
def consulta_XPO(fecha_inicio, fecha_fin):
    ...
    SELECT 
        RTRIM(AlbaranDoc) AS AlbaranDoc,
        FechaTeoricaServicioDoc AS [Delivery Date DD/MM/YYYY]
    FROM 
        vPendientesDistribucion
    WHERE 
        ID_Cliente = 944
        AND CONVERT(date, FechaEntrada) BETWEEN CONVERT(date, :inicio) AND CONVERT(date, :fin)
    """)
...
# --- EJECUTAR CONSULTA SQL SERVER ---
    ...
    if "Delivery Date DD/MM/YYYY_y" in df.columns:
        df["Delivery Date DD/MM/YYYY"] = df["Delivery Date DD/MM/YYYY_y"]
    ...
    if "Delivery Date DD/MM/YYYY" in df.columns and "Delivery instructions" in df.columns:
        idx = df.columns.get_loc("Delivery instructions") + 1 #type:ignore
        col = df.pop("Delivery Date DD/MM/YYYY")
        df.insert(idx, "Delivery Date DD/MM/YYYY", col)#type:ignore

    df.drop(columns=["AlbaranDoc"], inplace=True)

    # --- FORMATEAR FECHAS ---
    for col in ["Pickup Date DD/MM/YYYY", "Delivery Date DD/MM/YYYY"]:
    ...
    # # --- Diagnóstico antes de rellenar Destination Address Reference ---
    # print("Registros SQL Server:", len(df))
    # print("Primeros registros Destination Address – Name y Line 1:")
    # print(df[[col_nombre, col_line1]].head())

    # print("\nRegistros SQLite (clientes.db):", len(df_aux))
    # print("Primeros registros ABALPH, ALADD1, ALAN8:")
    # print(df_aux[["ABALPH", "ALADD1", "ALAN8"]].head())
    ...
    # print("\nClaves ABALPH disponibles para mapeo:", list(df_abalph.keys())[:5])
    # print("Claves ALADD1 disponibles para mapeo:", list(df_aladd1.keys())[:5])

    # 1️⃣ Match exacto ABALPH
    df_abalph = df_aux.drop_duplicates(subset="ABALPH").set_index("ABALPH")["ALAN8"].to_dict()
...
if __name__ == "__main__":
    consulta_XPO("2025-01-01", "2025-12-31")
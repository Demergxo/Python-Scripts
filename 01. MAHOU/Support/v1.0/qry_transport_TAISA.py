...
def consulta_taisa(fecha_inicio, fecha_fin):
    ...
    query = text("""
        ...
        ' ' AS 'Agrupación carga / Matrícula', ' ' AS 'Hora de Carga',' ' AS 'Tipo Doc', RTRIM(AlbaranDoc) AS 'AlbaranDoc', PedidoDestinatarioDoc AS 'Pedido Cliente',
        ...
        ' ' AS 'Dirección de entrega', ROUND(PesoFiege, 0) AS 'Peso Bruto', ROUND(BultosFiege, 0) AS 'Palés', '527' AS 'Estado', ' ' AS 'Peso Facturable (Real Taisa)',
    ...
    query_pend = text("""
    SELECT
        RTRIM(AlbaranDoc) AS AlbaranDoc, CampoCliente AS 'Tipo Doc', RTRIM(RutaReparto) AS 'Transporte'
    FROM    
        vDocumentos
    WHERE
        ID_Cliente = 944
    ...
    if "Transporte_y" in df.columns:
        df["Transporte"] = df["Transporte_y"]
    elif "Transporte" not in df.columns and "RutaReparto" in df.columns:
        df["Transporte"] = df["RutaReparto"]

    # --- UNIR TABLAS DE SQL SERVER ---
    for col in ["Tipo Doc", "Transporte"]:
    ...
    if "Tipo Doc" in df.columns and "Hora de Carga" in df.columns:
        idx = df.columns.get_loc("Hora de Carga") + 1 #type:ignore
        col = df.pop("Tipo Doc")
        df.insert(idx, "Tipo Doc", col) #type:ignore
    ...
    if "TRANSPORTE" in df.columns:
        ...
        df.loc[df["TRANSPORTE"] == "MSMALOXPO", "TRANSPORTE"] = "XPO"
...
if __name__ == "__main__":
    consulta_taisa("2025-01-01", "2025-12-31")
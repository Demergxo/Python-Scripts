...
def generar_codigos():
    ...
    for prefijo in ["AF", "CO", "FN", "R1", "R2", "RE", "RT"]:
        for i in range(1, 16):
            codigos.append(f"{prefijo}000{i:02}00")

    for i in range(1, 26):
        codigos.append(f"DE000{i:02}00")

    for i in range(1, 51):
        codigos.append(f"CA000{i:02}00")

    # MA formato: MA0100100
    fijo = "00"

    for bloque in range(1, 7):      # 01 a 6
        for i in range(1, 176):      # 001 a 0175
            codigos.append(f"MA{bloque:02}{i:03}{fijo}")
...
def obtener_directorio_para_codigo(codigo, directorio_base):
    ...
    if prefijo in ("MA", "PI", "BA") and len(codigo) >= 4:

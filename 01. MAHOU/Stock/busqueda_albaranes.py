...
def cargar_albaranes_excel(ruta_excel):
    """
    Lee los números de albarán desde un Excel.
    ...
    # Limpiar posibles espacios en los encabezados
    df_excel.columns = [
        str(col).strip()
        for col in df_excel.columns
    ]

    columnas_validas = ["Albarán", "Albaran", "AlbaranDoc"]

    columna_albaran = next(
        (col for col in columnas_validas if col in df_excel.columns),
        None
    )

    if columna_albaran is None:
        raise ValueError(
    ...
    if not albaranes:
        raise ValueError(
            "El Excel no contiene ningún número de albarán válido."
        )
...
def consulta_pedidos(fecha_inicio, fecha_fin, ruta_excel):
    date = time.strftime("%Y%m%d%H%M%S")

    albaranes_excel = cargar_albaranes_excel(ruta_excel)

    print(f"Albaranes leídos del Excel: {len(albaranes_excel)}")
...
#AND (ID_TipoDocumento = 1 OR ID_TipoDocumento = 3)
    ...
    with engine.connect() as conn:
        df1 = pd.read_sql(q1, conn, params={"albaranes": albaranes_excel}) #type: ignore

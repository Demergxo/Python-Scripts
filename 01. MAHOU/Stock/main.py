...
# --- Clase de Ventana Main ---
class VentanaMain(QWidget):
    def __init__(self):

        super().__init__()
        self.setObjectName("VentanaPrincipal")  # 👈 clave para volver a mostrar esta ventana
        ...
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
         ...
         # Etiqueta superior
        label = QLabel("Selección")
        label.setStyleSheet("font-size: 16px; font-weight: bold; margin-bottom: 20px;")
        layout.addWidget(label)

        # Botones
        self.btn_fechas = QPushButton("Stock")
        self.btn_crear = QPushButton("Caracteristicas")
        self.btn_modificar = QPushButton("Pedidos I/O")
        ...
        # Conexiones
        self.btn_fechas.clicked.connect(self.abrir_stock)
        self.btn_crear.clicked.connect(self.abrir_caracteristicas)
        self.btn_modificar.clicked.connect(self.abrir_pedidos)
    ...
    def abrir_caracteristicas(self):
        self.hide()
...
class VentanaFechas(QWidget):
    ...
    def aceptar_fechas(self):
        ...
        self.ventana_consultas = VentanaConsultas(inicio, fin)
...
class VentanaConsultas(QtWidgets.QWidget):
    def __init__(self, fecha_inicio, fecha_fin, parent=None):
    ...
    def ejecutar_consulta1(self):
        import qry_stock
        try:
            qry_stock.consulta_stock()
            QtWidgets.QMessageBox.information(self, "Fichero Stock", "Consulta de Stock ejecutada correctamente y generado archivo.")
        except Exception as e:
            error_dialog = QtWidgets.QErrorMessage(self)
            error_dialog.setWindowTitle("Fichero Stock")
            error_dialog.showMessage(f"Error en la consulta: servidor caído o sin permisos.\n\nDetalles: {e}")
            

    def ejecutar_consulta2(self):
        import qry_masterdata
        try:
            qry_masterdata.consulta_md()
            QtWidgets.QMessageBox.information(self, "Fichero MasterData", "Consulta MasterData ejecutada correctamente y generado archivo.")
        except Exception as e:
            error_dialog = QtWidgets.QErrorMessage(self)
            error_dialog.setWindowTitle("Fichero MasterData")
            error_dialog.showMessage(f"Error en la consulta: servidor caído o sin permisos.\n\nDetalles: {e}")


    def ejecutar_consulta3(self):
        import qry_pedidos_final
        try:
            qry_pedidos_final.consulta_pedidos(self.fecha_inicio, self.fecha_fin)
            QtWidgets.QMessageBox.information(self, "Fichero Info Pedidos", "Consulta Info Pedidos ejecutada correctamente y generado archivo.")
        except Exception as e:
            error_dialog = QtWidgets.QErrorMessage(self)
            error_dialog.setWindowTitle("Fichero Info Pedidos")
            error_dialog.showMessage(f"Error en la consulta: servidor caído o sin permisos.\n\nDetalles: {e}")
